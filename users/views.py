...
def pet_detail_view(request, pet_id):
 ...
 Displays detailed information for a pet listed for adoption (PetForAdoption model).
  This view is now public and does not require login.
 """
...
def admin_approve_report_view(request, report_id):
  ...
  if request.method == 'POST':
    ...
    if report.is_approved:
      messages.warning(request, f"Report # is already approved.")
      return redirect('users:admin_moderate_reports')

    report.is_approved = True
    report.save()
    messages.success(request,
             f"Report # ({report.report_type}) has been successfully approved and is now visible on the dashboard.")
    return redirect('users:admin_moderate_reports')
...
def admin_reject_report_view(request, report_id):
  ...
  if request.method == 'POST':
    ...
    # Delete the report
    report.delete()
    messages.success(request, f"Report # ({report.report_type}) has been successfully rejected and deleted.")
    return redirect('users:admin_moderate_reports')
