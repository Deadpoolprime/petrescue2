...
class Command(BaseCommand):
  ...
  def handle(self, *args, **options):
    ...
    # 1. Define the time threshold (15 days ago, using .date() for comparison with DateField)
    fifteen_days_ago = timezone.now().date() - datetime.timedelta(days=15)

    # 2. Find all 'Found' pet reports that are still 'Open' and older than 15 days
    eligible_reports = PetReport.objects.filter(
      report_type='Found',
      status='Open',
      event_date__lt=fifteen_days_ago
    )
    ...
    for report in eligible_reports:
      ...
      message = (
        ...
        f"found on {report.event_date.strftime('%Y-%m-%d')} is now eligible for adoption."
      )
